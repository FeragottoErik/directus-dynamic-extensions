...
export function checkFieldInTemplate(template: string, field: string) {
    // Remove `$CURRENT_ITEM.` if it exists
    const modifiedTemplate = template.replace(/\$CURRENT_ITEM\./g, '');

    // Find all instances of {{fieldName.subfield}} or similar patterns
    const matches = modifiedTemplate.match(/{{.*?}}/g);

    // Check if any match contains the field
