...
export function checkFieldInTemplate(template: Record<string, any>, field: string): boolean {
    // Iterate through each key in the template
    for (const key in template) {
        // Check if the field matches the key (including subfields separated by a dot)
